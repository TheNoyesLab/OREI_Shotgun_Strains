...
#Test run for three files only
coRead_list="$rundir/co_file.txt"
...
if [ -d "$assembly_path" ]; then
	...
	#Index scaffold file
	bwa index $assembly_path/final.contigs.fa
	

	#Make SAM of dehosted reads vs Scaffold
	bwa mem -t 40 $assembly_path/final.contigs.fa $coReads/$file.R1.fastq.gz $coReads/$file.R2.fastq.gz | samtools view -b > $work/$file.bam
	...
	metabat2 -i $assembly_path/final.contigs.fa -a $work/depth.txt -o $binning/metabat2/${file}_bins/${file}_metabat2_bin -t 40 -m 1500
...
assembly_path="$assemblies/coassemblies/megahit_${file}_output"
...
if [ -d "$assembly_path" ]; then
	...
	run_MaxBin.pl -contig $assembly_path/final.contigs.fa -reads $coReads/$file.R1.fastq.gz -reads2 $coReads/$file.R2.fastq.gz -thread 40 -min_contig_length 1500 -out $binning/maxbin2/${file}_bins/${file}_maxbin_bin
...
gen_coverage_file.sh -a $assembly_path/final.contigs.fa -o ${file}_bins -f .R1.fastq.gz -r .R2.fastq.gz -t 40 $coReads/$file.*.fastq.gz 
...
gen_kmer.py $assembly_path/final.contigs.fa 1500 4 
#Note that output goes to assembly location


#Create fasta that only keeps >1500 contigs
Filter_tooshort.py $assembly_path/final.contigs.fa 1500
...
bash run_metabinner.sh -a $assembly_path/final.contigs_1500.fa -o $output_dir -d $output_dir/coverage_profile_f1500.tsv -k $assembly_path/final.contigs_kmer_4_f1500.csv -p $path_to_MetaBinner -t 40
